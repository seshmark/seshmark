...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('seshmark.blame', runBlameCommand),
		vscode.commands.registerCommand('seshmark.query', runQueryCommand),
		vscode.commands.registerCommand('seshmark.who', runWhoCommand),
		vscode.commands.registerCommand('seshmark.toggle', toggleDecorations)
	);
...
async function getAttributions(filePath: string, cwd: string): Promise<LineAttribution[]> {
	// Run git blame --porcelain
	const { stdout } = await execAsync(`git blame --porcelain "${filePath}"`, { cwd });
	const lines = stdout.split('\n');

	const attributions: LineAttribution[] = [];
	let current: Partial<LineAttribution> = {};

	for (const line of lines) {
		if (line.match(/^[0-9a-f]{40}/)) {
			// New commit block
			if (current.line !== undefined) {
				attributions.push(current as LineAttribution);
			}
	...
	const commitMeta: Map<string, { session: string | null, agent: string | null, model: string | null }> = new Map();
...
function parseTrailers(body: string): { session: string | null, agent: string | null, model: string | null } {
...
function applyDecorations(editor: vscode.TextEditor, attributions: LineAttribution[]) {
	const config = vscode.workspace.getConfiguration('seshmark');
	const showGutter = config.get('showAgentInGutter', true);
	const colors = config.get<AgentColors>('colors', {});

	// Group by agent
	const byAgent: Map<string | null, LineAttribution[]> = new Map();
	for (const attr of attributions) {
	...
	for (const [agent, lines] of byAgent) {
		...
		if (agent && agent !== 'human') {
			...
			const options: vscode.DecorationOptions[] = lines.map(attr => {
